...
def add_log_section(page: ft.Page):
    ...
    Logger.init_logger(logging_scroll_box=log_text_scroll_box)
...
# tempo_core, tempo_cli tempo_tui, tempo_gui, tempo_web, tempo_installer, tempo_launcher
#
# tempo installer, manages installs, with various optional components, adds to path, etc..
#
# the following below files will mostly be pyc files so it can be dynamically populated and not bulk downloads
#
# tempo_launcher handles the logic to fire based on what pyc the installer installed
#   it will also have the main bundled python version that the pyc will be used with, ensuring compatibility
# tempo is the base lib
# tempo_cli is the click cli app
# tempo_tui is textual/trogon app that relies on the click cli app
# tempo_gui is flet gui app
# tempo_web is a flet web app, for things like browsers and unreal editor utility widgets
